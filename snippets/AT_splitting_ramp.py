...
par = dict({ 'dt': 4.E-11, 'tp': np.hstack((2.50E-8, np.ones(100)*4.E-11, 50.E-6)), 't0': 0.,
             'mu_ab': 1.+0.j, 'mu_bc': 1.+0.j, 'mu_ac':  1.+0.j,
             'E_a': 0., 'E_b': 2.460372e-24, 'E_c': 2.674924e-24,
             'o_ab': np.ones(102)*3713.17, 'o_bc': np.ones(102)*323.8, 'o_ac': np.ones(102)*4036.97,
             'F_ab': np.hstack((2000., np.linspace(2000, 0, 100), 0)), 'F_bc': np.hstack((0, np.linspace(0, 25, 100), 25)), 'F_ac': np.zeros(102),
             'phi_ab': np.zeros(102), 'phi_bc': np.zeros(102), 'phi_ac': np.zeros(102),
...
par['o_bc'] = np.ones(102)*323.8 + det
...
fid_n = fid_n[-1250000:]
np.save('long'+str(int(round(det*1000.))), fid_n)
